...
type TreeView struct {
	...
	onNodeItemSelect func(node *TreeNode)
}
...
func (tv *TreeView) SetOnNodeItemSelect(cb func(node *TreeNode)) {
	tv.onNodeItemSelect = cb
}
...
func (tv *TreeView) LoadChildren(node *TreeNode) {
	tv.loadChildren(node, true) // force reload
	tv.flattenVisible()
	if tv.OnChange != nil {
		tv.OnChange()
	}
...
// Lazy load children
func (tv *TreeView) loadChildren(node *TreeNode, force bool) {
	if !node.IsDir {
		return
	}
	if node.Children != nil && !force {
		return
	}
	...
	node.Children = nil // clear old children
...
func (tv *TreeView) Scroll(delta int) {
	...
	// Call onNodeItemSelect for the newly focused node
	node := tv.CurrentNode()
	if node != nil && tv.onNodeItemSelect != nil {
		tv.onNodeItemSelect(node)
	}
}
...
func (tv *TreeView) handleKey(ev *tcell.EventKey) {
	...
	case tcell.KeyEnter:
		if node.IsDir {
			...
			tv.ToggleNode(node)
		} else if tv.onNodeSelect != nil {
			tv.onNodeSelect(node.FullPath)
		}
		if tv.onNodeItemSelect != nil {
			tv.onNodeItemSelect(node)
		}
	case tcell.KeyRight:
		if node.IsDir && !node.Expanded {
